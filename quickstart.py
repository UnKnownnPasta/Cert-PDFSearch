...
#         self.output_label=tk.Label(self, text="Searching for PDF...")
#         self.output_label.pack()

        # self.download_button=tk.Button(self, text="Download", command=self.download_file)
        # self.download_button.pack()
        # # Update the label to show the file has been downloaded
        # self.output_label.config(text="File downloaded: " + filename)
...
#         if fname.endswith('.pdf'):
#             with open(os.path.join(directory, fname), 'rb') as file: # open file per iteration
#                 pdf=PyPDF2.PdfReader(file)
#                 for i in range(len(pdf.pages)):
#                     text=str(pdf.pages[i].extract_text())
#                     splitlist=text.split(' ')

#                     # Type 1 - search for name in extracted text
#                     if 'ANUPRIYA' in splitlist:
#                         print(file.name)

#                     # Type 2 - check without regard for case
#                     # for i in splitlist:
#                     #     if i.strip().lower().startswith('mat'):
#                     #         print(file.name)